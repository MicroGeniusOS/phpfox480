...
$Core.ProfilePhoto = {
  ...
  cropProfilePhoto: function (profileImage, serverId) {
    ...
    $('form', '#profile_crop_me').removeClass('hide');
  ...
  onSuccessUpload: function (ele, file, response) {
    if (typeof response === 'string') {
      ...
      if ($('.profile-image .profile_image_holder form label').length) {
        $('.profile-image .profile_image_holder form label').attr('onclick', '$Core.ProfilePhoto.update(\'' + response + '\', 0)');
      }
...
$Core.initClickAllLinks = function () {
  ...
  $('a, ._a').each(function () {
    if (!$(this).prop('built') && !$(this).hasClass('dont-unbind')) {
      $(this).off('click');
    }
  });
  ...
  setTimeout(function () {
    $('[data-component="menu"]:not(\'.built\')').each(function () {
      ...
      var th = $(this),
        ...
        check_dropdown_more = $('li.dropdown .dropdown-menu', th);
      //clear dropdown
...
$Behavior.globalInit = function () {
  ...
  $('.js_pager_view_more_link:not(.built)').each(function () {
    ...
    if ($(window).width() > 480 && iPageLoadMore <= oParams.iLimitLoadMore) {
...
$Behavior.disableButtonWhenClick = function () {
  let disabledCheckBtn = $(".js_core_action_button_disable_check");
