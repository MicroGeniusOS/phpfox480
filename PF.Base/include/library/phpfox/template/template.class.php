...
class Phpfox_Template
{
    ...
    private function _getBundleScript($mKey, $mValue)
    {
        switch ($mValue) {
            case 'style_script':
                return $this->getStyle('jscript', $mKey);
            case 'style_css':
                return $this->getStyle('css', $mKey);
            case 'static_script':
            ...
            default:
                if (preg_match('/app_/i', $mValue)) {
                    ...
                    if ($mKey == 'autoload.css') {
                        ...
                        $sFileName = trim(str_replace(dirname($this->_path_file) . PHPFOX_DS, '', $this->getStyle('css', $mKey, $aParts[1], true)), '/');
                    ...
                    if (isset($aParts[1]) && Phpfox::isModule($aParts[1])) {
                        if (substr($mKey, -3) == '.js') {
                            ...
                            $Theme = $this->theme()->get();
                            return $Theme->getCssFileName(str_replace($this->_path_file, '', $this->getStyle('css', $mKey, $aParts[1])));
                        }
                    }
                }
        }
    ...
    public function getHeader($bReturnArray = false)
    {
        ...
        if (!defined('PHPFOX_INSTALLER')) {
            ...
            if (!Phpfox::isAppActive('Core_Messages') && !Phpfox::isAppActive('PHPfox_IM') && !Phpfox::isAppActive('P_ChatPlus')) {
        ...
        if (!PHPFOX_IS_AJAX_PAGE) {
            ...
            $aJsVars = [
                ...
                'sImagePath'       => $oAssets->getAssetUrl($this->getStyle('image'), false),
                'sStylePath'       => $oAssets->getAssetUrl($this->getStyle('css'), false),
                    ...
                    switch ($mValue) {
                        case 'style_script':
                            if ($bReturnArray) {
                                ...
                                if ($bToHead == 'head') {
                                    $aCacheCSS[] = str_replace($this->_path_file, '', $this->getStyle('jscript', $mKey));
                                } else {
                                    $aCacheJs[] = str_replace($this->_path_file, '', $this->getStyle('jscript', $mKey));
                                }
                        ...
                        case 'style_css':
                            if ($bReturnArray) {
                                ...
                                $aCacheCSS[] = str_replace($this->_path_file, '', $this->getStyle('css', $mKey));
                            }
                            ...
                            if (preg_match('/app_/i', $mValue)) {
                                ...
                                if (isset($aParts[1]) && Phpfox::isModule($aParts[1])) {
                                    if (substr($mKey, -3) == '.js') {
                                        ...
                                        $Theme = $this->theme()->get();
                                        $sFileName = $Theme->getCssFileName(str_replace($this->_path_file, '', $this->getStyle('css', $mKey, $aParts[1])));
                                        $aCacheCSS[] = $sFileName;
                                    }
                                }
                            }
    ...
    public function addScript($key, $value, $bReturnString = true)
    {
        ...
        switch ($value) {
            case 'style_script':
                if ($bReturnString) {
                    return '<script src="' . $oAssets->getAssetUrl($this->getStyle('jscript', $key)) . '"></script>';
                }
                $this->_sAddScripts .= '<script src="' . $oAssets->getAssetUrl($this->getStyle('jscript', $key)) . '"></script>';
                break;
            case 'style_css':
                if ($bReturnString) {
                    return '<link type="text/css" rel="stylesheet" href="' . $oAssets->getAssetUrl($this->getStyle('css', $key)) . '"/>';
                }
                $this->_sAddScripts .= '<link type="text/css" rel="stylesheet" href="' . $oAssets->getAssetUrl($this->getStyle('css', $key)) . '"/>';
                break;
            case 'static_script':
                if ($bReturnString) {
            ...
            default:
                if (preg_match('/app_/i', $value)) {
                    ...
                    if ('.js' == substr($key, -3)) {
                        ...
                        $Theme = $this->theme()->get();
                        $sFileName = trim(str_replace(dirname($this->_path_file) . PHPFOX_DS, '', $this->getStyle('css', $key, $aParts[1], true)), '/');
                        $sFileName = $Theme->getCssFileName($sFileName, 'app');
                    ...
                    if (isset($aParts[1]) && Phpfox::isModule($aParts[1])) {
                        if (substr($key, -3) == '.js') {
                            ...
                            $Theme = $this->theme()->get();
                            $sFileName = $Theme->getCssFileName(str_replace($this->_path_file, '', $this->getStyle('css', $key, $aParts[1])));
    ...
    public function getMenu($sConnection = null)
    {
        ...
        foreach ($aMenus as $iKey => $aMenu) {
            ...
            if (defined('PHPFOX_IS_PAGES_VIEW')) {
                ...
                if (defined('PHPFOX_PAGES_ITEM_TYPE') && PHPFOX_PAGES_ITEM_TYPE == $aMenu['module']) {
