...
class Phpfox_Parse_Output
{
    ...
    private function _parseGroupMentioned($iGroupId)
    {
        ...
        $iUserId = $oService->getUserId($iGroupId);
        if (isset($aGroup['title'])) {
            $sOut = '<span class="user_profile_link_span" id="js_user_name_link_' . $iUserId . '"><a class="status_user_tag" href="' . $sUrl . '">' . $aGroup['title'] . '</a></span>';
        }
        return $sOut;
    }
    ...
    public function shorten($html, $maxLength, $sSuffix = null, $bHide = false)
    {
        ...
        if ($sSuffix !== null) {
            ...
            if (mb_strlen($sCountHtml) > $maxLength || $numLine > 5) {
                $sSuffix = _p($sSuffix);
    ...
    public function split($sString, $iCount, $bBreak = false)
    {
        ...
        $sOut = ($bHasNonAscii === true ? str_replace(['[PHPFOX_START]', '[PHPFOX_END]'], ['&lt;', '&gt;'],
