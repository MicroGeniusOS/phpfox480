        ...
        if (($aVals = $this->request()->getArray('val')) || $bAjaxUpload) {
            if (isset($aVals['crop-data'])) {
                ...
                if (isset($aUserImage) && isset($aUserImage['user_image'])) {
                    ...
                    $serverId = $sUserImage['server_id'];
                } else {
                    $sUserImage = Phpfox::getUserBy('user_image');
                    $serverId = Phpfox::getUserBy('server_id');
                }
                Phpfox::getLib('cdn')->setServerId($serverId);
                ...
                $sOriginalUserImagePath = Phpfox::getParam('core.dir_user') . sprintf($sUserImage, '');
                $oFile = Phpfox_File::instance();
                $oFile->unlink($sOriginalUserImagePath);
                $oFile->copy($sTempPath, $sOriginalUserImagePath);
                Phpfox::getLib('cdn')->put($sTempPath);
                @unlink($sTempPath);
