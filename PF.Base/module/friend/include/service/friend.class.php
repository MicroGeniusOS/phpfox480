...
class Friend_Service_Friend extends Phpfox_Service
{
     ...
     */
    public function getFromCache($mAllowCustom = false, $sUserSearch = false, $bIncludeCurrentUser = false, $userId = null)
    {
        ...
        foreach ($aRows as $iKey => $aRow) {
            if (!$this->_checkPrivacy($aRow, $userId)) {
                unset($aRows[$iKey]);
                continue;
            }
     ...
     * @return bool
     */
    private function _checkPrivacy($aRow, $userId)
    {
        $valid = true;
        if ($userId == $aRow['user_id']) {
            $valid = false;
        }
        if ($valid && Phpfox::isAppActive('Core_Messages')
    ...
    public function getMutualFriends($iUserId, $iLimit = 7, $bNoCount = false)
    {
        ...
        $aCache[$iUserId . '_' . $iLimit] = [$iCnt, $aRows];
    ...
    public function isFriendOfFriend($iUserId)
    {
        ...
        list($iCnt,) = $this->getMutualFriends($iUserId);
