...
$Behavior.tableHover = function () {
  ...
  if($Core.exists('#admincp_search_input')) {
  ...
  if($Core.exists('#js_check_box_all')) {
...
$Behavior.custom_admin_init = function () {
    $('.js_drop_down').off().click(function () {
        ...
        $('#js_cache_menu .link_menu li a').each(function () {
            var sId = '#js_' + aParams['type'] + '_' + aParams['id'];
            if ($(this).attr('href') == "#active") {
                if ($(sId).length > 0 && $(sId).find('del').length > 0) {
                    $(this).html(oTranslations['set_to_active']);
                }
                else {
                    $(this).html(oTranslations['set_to_inactive']);
                }
            }
    ...
    $('.var_type').off().change(function () {
        var optionHolder = $('#tbl_option_holder'),
            customOption = $('#tbl_add_custom_option');
        ...
        switch (this.value) {
            ...
            case 'checkbox':
                optionHolder.show();
                optionHolder.closest('.panel').show();
                customOption.show();
                break;
            default:
                optionHolder.hide();
                optionHolder.closest('.panel').hide();
                customOption.hide();
                break;
        }
    });
    ...
    $('.js_add_custom_option').off().click(function () {
        $Core.custom.add();

        return false;
    });

    $('#js_create_new_group').off().click(function () {
    ...
    $('#js_cancel_new_group').off().click(function () {
    ...
    $('.js_delete_current_option').off().click(function () {
        var obj = this;
    ...
    $('.js_custom_change_group').off().click(function () {
