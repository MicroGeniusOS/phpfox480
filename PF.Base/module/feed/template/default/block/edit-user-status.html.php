        ...
        <div class="activity_feed_form_button" style="display: block">
            ...
            {if $bLoadCheckIn}
                <div id="js_location_input{$aForms.feed_id}">
                    <a class="btn btn-danger pr-4" href="#" title="{_p var='close'}" onclick="$Core.FeedPlace.cancelCheckIn({$aForms.feed_id}, true); return false;"><i class="fa fa-eye-slash"></i></a>
            ...
            {if Phpfox::getParam('feed.enable_check_in') && Phpfox::getParam('core.google_api_key') != ''}
                <div id="js_add_location{$aForms.feed_id}">
                    <div><input type="hidden" id="val_location_latlng{$aForms.feed_id}" name="val[location][latlng]" {if !empty($aForms.location_latlng)}value="{$aForms.location_latlng.latitude},{$aForms.location_latlng.longitude}"{/if}></div>
