...
class Feed_Service_Callback extends Phpfox_Service
{
    ...
    public function getActivityFeedComment($aItem)
    {
        ...
        if (!empty($aRow['parent_user_name']) && empty($_POST)) {
            $aReturn['parent_user'] = Phpfox::getService('user')->getUserFields(true, $aRow, 'parent_');
        }

        // get tagged users
        $aReturn['total_friends_tagged'] = Phpfox::getService('feed')->getTaggedUsers($aItem['item_id'], 'feed_comment', true);
        if ($aReturn['total_friends_tagged']) {
            $aReturn['friends_tagged'] = Phpfox::getService('feed')->getTaggedUsers($aItem['item_id'], 'feed_comment', false, 1, 2);
        }
    ...
    public function addComment($aVals, $iUserId = null, $sUserName = null)
    {
        ...
        $aTaggedByWith = Phpfox::getService('feed')->getTaggedUserIds($aRow['feed_comment_id'], 'feed_comment');

        // check tag privacy
        Phpfox::getService('feed')->filterTaggedPrivacy($aMentions, $aTaggedByWith);
    ...
    public function addLikeComment($iItemId, $bDoNotSendEmail = false)
    {
        ...
        if (!$bDoNotSendEmail) {
            ...
            Phpfox::getLib('mail')->to($aRow['user_id'])
                ->subject(['user.full_name_liked_a_comment_you_posted_on_row_full_name_s_wall', ['full_name' => Phpfox::getUserBy('full_name'), 'row_full_name' => $aRow['full_name']]])
                ->message(['user.full_name_liked_your_comment_message', ['full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'content' => Phpfox::getLib('parse.output')->shorten($aRow['content'], 50, '...'), 'row_full_name' => $aRow['full_name']]])
    ...
    public function addLikeMini($iItemId, $bDoNotSendEmail = false)
    {
        ...
        if (!$bDoNotSendEmail) {
            ...
            Phpfox::getLib('mail')->to($aRow['user_id'])
                ->subject(['user.full_name_liked_one_of_your_comments', ['full_name' => Phpfox::getUserBy('full_name')]])
                ->message(['user.full_name_liked_your_comment_message_mini', ['full_name' => Phpfox::getUserBy('full_name'), 'link' => $sLink, 'content' => Phpfox::getLib('parse.output')->shorten($aRow['text'], 50, '...')]])
                ->notification('like.new_like')
                ->send();
    ...
    public function getNotificationMini_Like($aNotification)
    {
        ...
        if (!$aRow) {
            ...
            $sPhrase = _p('users_liked_your_comment_text_that_you_posted', ['users' => Phpfox::getService('notification')->getUsers($aNotification), 'text' => Phpfox::getLib('parse.output')->shorten($aRow['text'], Phpfox::getParam('notification.total_notification_title_length'), '...')]);
            return [
