...
$Core.Feed =
  {
    ...
    init: function (bForce) {
      ...
      $('#' + $Core.Feed.sButtonId).click(function () {
        bCheckinInit = true;

        if ($(this).closest('#js_activity_feed_edit_form').length) {
          $('#' + $Core.Feed.sMapId).hide();
        }
        ...
        $($Core.Feed).on('mapCreated', function () {
          $Core.Feed.showMap();
        });
      ...
      $($Core.Feed).on('gotVisitorLocation', function () {
        $Core.Feed.createMap();
      });
    ...
    generateFeedMap: function () {
      PF.waitUntil(function () {
        ...
        $('[data-component="pf_map"]:not(\'.built\')').each(function () {
          ...
          $Core.Feed.aHoverPlaces[th.data('id')] = {
          ...
          $Core.Feed.gMarker = new google.maps.Marker({
            map: $Core.Feed.aHoverPlaces[th.data('id')]['map'],
            position: gLatLng,
            draggable: true,
            animation: google.maps.Animation.DROP
          });

          google.maps.event.trigger($Core.Feed.aHoverPlaces[th.data('id')]['map'], 'resize');
    ...
    getVisitorLocation: function (sFunction) {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(function (oPos) {
          ...
          function () {
            $Core.Feed.getLocationWithoutHtml5(sFunction);
          }
        );
      }
      else {
    ...
    getLocationWithoutHtml5: function (sFunction) {
      ...
      if (sCookieLocation != null) {
        ...
        switch (sFunction) {
          case 'showMap':
            sParams += '&callback=$Core.Feed.showMap';
            break;
          case 'createMap':
            sParams += '&callback=$Core.Feed.createMap';
            break;
        }
    ...
    createMap: function () {
      ...
      $Core.Feed.gGeoCoder.geocode({'latLng': $Core.Feed.gMyLatLng}, function (oResults, iStatus) {
        if (iStatus == google.maps.GeocoderStatus.OK && oResults[1]) {
          ...
          $('#val_location_latlng').val(oResults[1].geometry.location.lat() + ',' + oResults[1].geometry.location.lng());
    ...
    chooseLocation: function (id) {
      ...
      $('.js_location_feedback').html(oTranslations['at_location'].replace('{location}', oPlace['name'])).show().addClass('active');
      $('#' + $Core.Feed.sButtonId).removeClass('is_active');
      $('#js_add_location_suggestions, #js_feed_check_in_map, #js_location_input').hide();
      $('.activity_feed_form_button_position').show();
      if ($sCurrentForm == 'global_attachment_photo' || $sCurrentForm == 'custom') {
    ...
    googleReady: function (sGoogleKey) {
      ...
      script.src = "https://maps.google.com/maps/api/js?libraries=places&key=" + sGoogleKey + "&callback=$Core.Feed.init";
    ...
    cancelCheckIn: function () {
      ...
      $('.js_location_feedback').html('').removeClass('active');
...
$Core.FeedPlace =
  {
    ...
    init: function (bForce) {
      ...
      $('#' + _this.matchIdIndex(_this.sButtonId)).off('click').on('click',function () {
      ...
      $(_this.matchIdIndex('#hdn_location_name')).focus( function () {
          ...
          _this.gMap[_this.sCurrentIndex].setCenter(_this.gMyLatLng);
        })
        .on('keyup', function () {
          var sName = $(this).val();
          ...
          for (var i in _this.aPlaces) {
            if (typeof _this.aPlaces[i]['is_auto_suggested'] != 'undefined' && _this.aPlaces[i]['is_auto_suggested']) {
              _this.aPlacesKey.splice($.inArray(_this.aPlaces[i]['id'], _this.aPlacesKey), 1);
          ...
          _this.gSearch[_this.sCurrentIndex].nearbySearch(
            ...
            function (results, status) {
              if (status === google.maps.places.PlacesServiceStatus.OK) {
                results.map(function (oResult) {
                  if ($.inArray(oResult['id'], _this.aPlacesKey) == '-1') {
                    oResult['is_auto_suggested'] = true;
                    _this.aPlaces.push(oResult);
                    _this.aPlacesKey.push(oResult['id']);
                  }
                });
              }
        ...
        .on('focus', function () {
          if ($(_this.matchIdIndex('#js_add_location_suggestions')).is(':visible') !== true) {
    ...
    displaySuggestions: function () {
      ...
      _this.aPlaces.map(function (oPlace) {
        sOut += '<div class="js_div_place" onmouseover="$Core.FeedPlace.hintPlace(\'' + oPlace['id'] + '\', \''+ _this.sCurrentIndex +'\');" onclick="$Core.FeedPlace.chooseLocation(\'' + oPlace['id'] + '\', \''+ _this.sCurrentIndex +'\');">';
        sOut += '<div class="js_div_place_name">' + oPlace['name'] + '</div>';
        if (typeof oPlace['vicinity'] != 'undefined') {
          sOut += '<div class="js_div_place_vicinity">, ' + oPlace['vicinity'] + '</div>';
        }
      ...
      $(_this.matchIdIndex('#js_add_location_suggestions')).html(sOut).css({'z-index': 900, 'max-height': '150px'}).show();
    },
    ...
    /* Visually accepts a suggestion and sets the internal value for the form*/
    chooseLocation: function (id, index) {
      var oPlace = false, _this = this;
      this.aPlaces.map(function (oCheck) {
        if (oCheck['id'] == id) {
          oPlace = oCheck;
          return;
        }
      ...
      if (typeof oPlace['latitude'] != 'undefined') {
        $(_this.matchIdIndex('#val_location_latlng', index)).val(oPlace['latitude'] + ',' + oPlace['longitude']);
      }
      else if (typeof oPlace['geometry'] != 'undefined') {
        $(_this.matchIdIndex('#val_location_latlng', index)).val(oPlace['geometry']['location'].lat() + ',' + oPlace['geometry']['location'].lng());
      }
    ...
    storePlaces: function (jPlaces, oCallback) {
      ...
      $(oPlaces).each(function (iPlace, oNewPlace) {
        ...
        if (bAddPage) {
          ...
          _this.aPlacesKey.push(oNewPlace['id']);
        }
      });
      ...
      this.gSearch[this.sCurrentIndex].nearbySearch({
        ...
        if (iStatus == google.maps.places.PlacesServiceStatus.OK) {
          for (var i = 0; i < aResults.length; i++) {
            ...
            $Core.FeedPlace.aPlacesKey.push(aResults[i]['id']);
          }
        }
        $Core.FeedPlace.displaySuggestions();
      });
    },
    ...
    googleReady: function (sGoogleKey, initIndex) {
      ...
      var script = document.createElement("script");
      script.type = "text/javascript";
      script.src = "https://maps.google.com/maps/api/js?libraries=places&key=" + sGoogleKey + "&callback=$Core.FeedPlace.init";
      document.body.appendChild(script);
    ...
    cancelCheckIn: function (index, bClose) {
      ...
      if (!bClose) {
        ...
        $(this.matchIdIndex('#hdn_location_name', index) + ',' + this.matchIdIndex('#val_location_name', index) + ',' + this.matchIdIndex('#val_location_latlng', index)).val('');
      }
    }
  };
